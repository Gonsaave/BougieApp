package com.example.gonzalo.bougieapp;
    ...
    private UVCCameraHelper.OnMyDevConnectListener listener = new UVCCameraHelper.OnMyDevConnectListener() {
        ...
        public void onAttachDev(UsbDevice device) {
            if (mCameraHelper == null || mCameraHelper.getUsbDeviceCount() == 0) {
                showShortMsg("No se ha conectado dispositivo");
                return;
            }
        ...
        public void onDettachDev(UsbDevice device) {
            ...
            if (isRequest) {
                ...
                showShortMsg(device.getDeviceName() + " está desconectado");
            }
        }
        ...
        public void onConnectDev(UsbDevice device, boolean isConnected) {
            if (!isConnected) {
                showShortMsg("Fallo de conexión, por favor revise parametros de resolución");
                isPreview = false;
            } else {
                isPreview = true;
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            case R.id.menu_takepic:
                if (mCameraHelper == null || !mCameraHelper.isCameraOpened()) {
                    showShortMsg("Hubo un error al reproducir la camara.");
...
//                                FileUtils.putFileStream(data, offset, length);
                    ...
                    showShortMsg("Iniciar grabación...");
                    // mSwitchVoice.setEnabled(false);
                } else {
//                    FileUtils.releaseFile();
                    mCameraHelper.stopRecording();
                    showShortMsg("Detener grabación...");
    ...
    public void onDialogResult(boolean canceled) {
        if (canceled) {
            showShortMsg("Se ha cancelado la acción.");
        }
    }
